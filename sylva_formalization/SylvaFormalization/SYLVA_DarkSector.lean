...
def YinYangDarkSector.darkSectorIsYinPole (y : YinYangDarkSector) : Prop :=
    y.darkMatterAsYin ∧ y.darkEnergyAsYang

/-- **Theorem (Yin-Yang Dark Sector)**: The dark sector is the Yin pole of
...
theorem yin_yang_dark_sector (y : YinYangDarkSector) :
    y.darkSectorIsYinPole ↔ y.darkMatterAsYin ∧ y.darkEnergyAsYang := by rfl
...
def YinStructure.explainsDarkMatter (s : YinStructure) : Prop :=
    s.defectDensity > 0

/-- **Theorem (Dark Matter as Yin Structure)**: Dark matter is not a particle
...
theorem dark_matter_as_yin_structure (s : YinStructure) :
    s.explainsDarkMatter ↔ s.defectDensity > 0 := by rfl
...
def YangExpansion.explainsDarkEnergy (y : YangExpansion) : Prop :=
    y.equationOfState = -1

/-- **Theorem (Dark Energy as Yang Expansion)**: Dark energy is the Yang
...
theorem dark_energy_as_yang_expansion (y : YangExpansion) :
    y.explainsDarkEnergy ↔ y.equationOfState = -1 := by rfl
...
def UnifiedDarkSector.unified (u : UnifiedDarkSector) : Prop :=
    u.yinStructure.explainsDarkMatter ∧ u.yangExpansion.explainsDarkEnergy

theorem unified_dark_sector (u : UnifiedDarkSector) :
    u.unified ↔ u.yinStructure.explainsDarkMatter ∧ u.yangExpansion.explainsDarkEnergy := by rfl
