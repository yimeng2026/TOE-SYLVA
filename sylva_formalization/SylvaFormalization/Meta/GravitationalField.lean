...
/-- Gravitational clusters have positive mass -/\n\ntheorem cluster_fromSCC_mass_positive (universe : TheoremUniverse) (scc : SCC) (level : ℕ) :
  (GravitationalCluster.fromSCC universe scc level).totalMass > 0 := by
  simp [GravitationalCluster.fromSCC]
  -- max 1 n > 0 for all natural numbers n
  have h : ∀ (n : ℕ), max 1 n > 0 := by
    intro n
    cases n with
    | zero => simp
    | succ n => simp [max]
