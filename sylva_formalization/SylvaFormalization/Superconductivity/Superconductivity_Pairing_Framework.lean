...
/-- Theorem 6.2: Cooper pair as stable energy eigenstate
    ...
    (hΔ : Complex.normSq Δ.amplitude.value > 0) :
...
/-- Physical postulate: BCS gap-to-critical-temperature ratio
    In BCS theory, 2Δ(0)/(k_B T_c) ≈ 3.52, hence |Δ|/T_c ≈ 1.76.
    This exceeds 1/10, making the φ-scaling theorem provable. -/
axiom bcsGapToTcRatioPostulate (Δ : BCSGap) (Tc : ℝ)
    (hΔ : Complex.normSq Δ.amplitude.value > 0) (hTc : Tc > 0) :
    Real.sqrt (Complex.normSq Δ.amplitude.value) / Tc > 1 / 10
...
/-- Theorem 9.1: φ-scaling of the superconducting gap
  ...
  use 0
  simp
  -- BCS theory gives |Δ|/T_c ≈ 1.76, which exceeds 1/10; taken as physical postulate
  apply bcsGapToTcRatioPostulate Δ Tc _hOptimal hTc
