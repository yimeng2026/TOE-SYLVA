...
def TemporalRecurrence {S : StratifiedSpace N} (C_t : ∀ k, ℔
  ...
  -- 范数守恒（受扰动）：质量累积的单调  ?  --   ?v ≔C_t/(m_P·K_loop) 时，近似守恒；需精确分析扰动  ?  normPreserving := postulate
...
def SpatialRecurrence {S : StratifiedSpace N} (C_s : ∀ k, ℔
  ...
  -- 范数守恒（受扰动）：手性差异的可逆  ?  -- 空间分量可正可负，需分析 χ 的符号对范数的影  ?  normPreserving := postulate
  -- 线性近似：  ?‖v—小时，递推近似为恒等映  ?  linearApprox := postulate
