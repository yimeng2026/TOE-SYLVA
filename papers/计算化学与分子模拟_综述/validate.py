...
def module_02_md_verlet():
    ...
    k = 10.0          # 弹簧常数
    m = 1.0           # 质量
    # Velocity Verlet 是辛积分器，无长期漂移，但能量存在振幅
    # ~(omega*dt)^2/4 的周期性振荡 (omega = sqrt(k/m))。
    # 要使端点相对偏差 < 1e-6，需 (omega*dt)^2/4 < 1e-6，
    # 即 dt < 2e-3/omega ≈ 6.3e-4；取 dt = 5e-4 并等比增加步数，
    # 保持总模拟时长 t_max = 100 不变。
    dt = 5.0e-4       # 时间步长
    n_steps = 200000  # 总步数 (t_max = 100 不变)
    x0, v0 = 1.0, 0.0  # 初始条件
...
def module_05_umbrella_wham():
    ...
    k = 10.0
    # 伞形采样重加权的前提: 偏置窗口须与目标(无偏)分布有显著重叠。
    # 原参数 k_bias=50, x0_bias=1.5 使有偏样本集中在 1.25±0.13,
    # 而无偏分布 N(0, 1/sqrt(k)) 的峰值在 0、sigma≈0.32,
    # 两者几乎零重叠, 重要性重加权在数学上无法恢复无偏均值。
    # 取 k_bias=10, x0_bias=0.5: 有偏分布 N(0.25, 1/sqrt(20)) 与
    # 无偏分布充分重叠, 满足重加权前提。
    k_bias = 10.0
    x0_bias = 0.5
    # 力常数与偏置中心均为 O(1) 的约化量，能量应以 k_B T 为单位。
    # 若直接取 SI 单位 k_B T ≈ 4.1e-21 J，热涨落 sigma ~ 1e-12 与
    # O(1) 的偏置中心不自洽，beta*U_bias ~ 1e21 使 exp 上溢为 nan。
    # 采用约化单位 k_B T = 1（即 beta = 1）。
    kB_T = 1.0        # 热能，约化单位
    beta = 1.0 / kB_T
