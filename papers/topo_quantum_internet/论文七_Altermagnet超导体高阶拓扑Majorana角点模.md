...
Hodge 等人 [1] 已通过含时数值模拟论证，在 AM-SC 方形平台上通过 Néel 矢量旋转可实现 √Z 门，在 H-junction 七平台架构上可实现 √X 门。本文的数值结果直接支持该方案：Néel 矢量旋转确实能将角点模沿边界移动（图 3(b) → (c)），而在完整 2π 旋转后系统恢复初始状态（图 3(a) 的周期性）。下一步需在更大晶格（L ≥ 22）和更长编织时间（T ≥ 10⁴ ℏ/t）下模拟绝热编织的完整动力学，以验证非阿贝尔统计的保真度。
