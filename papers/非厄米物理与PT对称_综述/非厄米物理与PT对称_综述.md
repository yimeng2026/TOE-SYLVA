...
$\mathcal{P}$ 是空间反射，$\mathcal{T}$ 是时间反演（同时复共轭）。厄米哈密顿量 $H=p^2+V(x)$ 在 $V(x)=V(x)^*$（即 $V$ 是实函数）下保持 $\mathcal{T}$ 对称。若同时 $V(x)=V(-x)$，则进一步有 $\mathcal{P}$ 对称。但 $\mathcal{PT}$ 联合对称允许 $V$ 既非实也非偶——例如 $V(x)=ix^3$：$\mathcal{P}$ 把 $x^3\to -x^3$，$\mathcal{T}$ 把 $i\to -i$，因此 $\mathcal{PT}(ix^3)=\mathcal{P}(-ix^3)=i(-x)^3=ix^3$，保持不变。
...
- **PT 对称 XY 链**：通过 Jordan-Wigner 变换可严格求解。
...
$\mathcal{PT}$ 多体系统的稳定相存在一些"反对称耦合"导致的"No-Go 禁戒"：在某些 PT 对称自旋链中，能谱复化与基态纠缠熵尖峰同步出现，对应 $\mathcal{PT}$ 破缺的多体相变，可视为 $\mathcal{PT}$ 推广的 ETH-MBL 过渡[^ashida-zhang-2023]。
...
在 LC 振荡电路、声学腔体中，电阻正负耦合实现 PT 等效结构，可做拓扑开关与声学隐身。
...
**普适对称性**断言：所有物理上实现的对称性是同一个底层结构的投影。$\mathcal{PT}$ 联合反演是 $\mathbb{Z}_2$ 反演的特定投影：把 $\mathbb{Z}_2$ 反演分解为"空间定向反转（$\mathcal{P}$）"与"时间定向反转（$\mathcal{T}$）"两极，二者在量子-引力体上等价（CPT 定理），但在非相对论极限下可独立破缺。$\mathcal{PT}$ 对称的破缺—未破缺相变，是 $\mathbb{Z}_2$ 反演在量子力学层级的对称破缺相变，属于普适对称性投影的"局部层级"分支。
...
| 层级涌现 | PT 破缺相变 / EP Jordan 块 | 对称—可观测—几何三层相变 | M2 |
