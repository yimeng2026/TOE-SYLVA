...
def verify_molmer_sorensen_gate():
    ...
    # FIXED: MS gate uses H = X1 X2 (product, not square-of-sum)
    # U_ms = exp(-i * pi/4 * X1 X2)  produces Bell state from |00>
    H_ms = np.kron(sx, sx)  # X1 X2
    theta = np.pi / 4  # pi/4 gives maximal entanglement
    
    # Construct unitary
    U_MS = scipy_exp_if_available(-1j * theta * H_ms)
    
    # Verify: |00> -> (|00> - i|11>)/sqrt(2)
    psi_00 = np.array([1, 0, 0, 0])  # |00>
    psi_out = U_MS @ psi_00
    
    # Bell state: (|00> - i|11>)/sqrt(2)
    ...
    print("=" * 60)
    print(f"MS gate H = X1*X2, theta = {theta:.4f} (pi/4)")
    print(f"Output Bell state fidelity: {fidelity:.6f}")
    print(f"[PASS] Verification passed" if fidelity > 0.999 else "[FAIL] Verification failed")
    print()
