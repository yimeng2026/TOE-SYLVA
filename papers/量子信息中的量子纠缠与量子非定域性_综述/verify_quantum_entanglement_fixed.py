...
def verify_chsh_inequality() -> dict:
    ...
    b_prime = (PAULI_X - PAULI_Z) / np.sqrt(2)  # FIXED: was (Z-X), correct CHSH needs (X-Z)
...
def verify_steering_inequality() -> dict:
    ...
    steering_threshold = 0.5  # Werner steering boundary is p > 1/2; p=0.5 is exactly at boundary
    ...
    for p in p_values:
        ...
        results[f'werner_p={p:.4f}'] = {
            ...
            'expected_steering': p > steering_threshold + 1e-10  # FIXED: strict > for steering,
        }
