...
class FullMagazine:
    ...
    def parse_md_to_story(self, filepath):
        ...
        for line in lines:
            ...
            if line.startswith('```'):
                if in_code_block:
                    if code_content:
                        code_text = '<br/>'.join(code_content[:100])  # 限制100行
            ...
            if line.startswith('# '):
                ...
                self.story.append(Paragraph(f"<b>{text}</b>", self.styles['CustomBody']))
            elif line.startswith('- ') or line.startswith('* '):
                text = self.clean_markdown(line[2:])
                self.story.append(Paragraph(f"• {text}", self.styles['CustomBody']))
...
╚════════════════════════════════════════════════╝"""
        ...
        logo_style = ParagraphStyle(name='Logo', fontName='Courier', fontSize=8, leading=10, alignment=TA_CENTER)
        self.story.append(Paragraph(f"<font name='Courier' size='8'>{logo.replace(chr(10), '<br/>')}</font>", logo_style))
        self.story.append(Spacer(1, 15*mm))
        
        self.story.append(Paragraph("<b>室温超导特刊</b>", self.styles['MagTitle']))
        subtitle_style = ParagraphStyle(name='Subtitle', fontName=chinese_font, fontSize=14, leading=20, alignment=TA_CENTER)
        self.story.append(Paragraph("Room-Temperature Superconductivity<br/>", subtitle_style))
   ...
   LaSc₂H₂₄ @ 298 K"""
        crystal_style = ParagraphStyle(name='Crystal', fontName='Courier', fontSize=9, leading=12, alignment=TA_CENTER, textColor=colors.HexColor('#555555'))
        self.story.append(Paragraph(f"<font name='Courier' size='9'>{crystal.replace(chr(10), '<br/>')}</font>", crystal_style))
        
        self.story.append(Spacer(1, 20*mm))
        self.story.append(Paragraph("作者：一梦老师<br/>编辑：一梦老师<br/>审核：一梦老师<br/><br/>Sʏʟᴠᴀ系列杂志工作室<br/>VOL.001 | 2026年4月", self.styles['QuoteBox']))
    ...
    def add_toc(self):
        ...
        toc = [
            ...
            ("第2章 理论基础", "BCS理论到Eliashberg", "P25"),
            ("第3章 必然性定理 ★", "满足三条件→室温超导必然", "P55"),
            ("第4章 材料预测", "LaSc₂H₂₄的双笼结构", "P85"),
            ("第5章 实验突破 ★", "2025年10月历史性时刻", "P115"),
            ("第6章 形式化框架", "Lean机器验证", "P145"),
            ("第7章 未来展望", "超导时代正在开启", "P165"),
            ("附录", "公式速查 / 参考文献", "P185"),
        ]
        
        for title, desc, page in toc:
            self.story.append(Paragraph(f"<b>{title}</b>", self.styles['SectionTitle']))
            self.story.append(Paragraph(f"└─ {desc} ........................ {page}", self.styles['CustomBody']))
    ...
    def build(self):
        ...
        <b>误差仅6% —— 这是必然性的终极证明</b>
        """
        self.story.append(Paragraph(theorem_content, self.styles['CustomBody']))
        self.story.append(PageBreak())
        ...
        if lean_file.exists():
            ...
            code_style = ParagraphStyle(name='LeanCode', fontName='Courier', fontSize=8, leading=10)
            self.story.append(Paragraph(f"<font name='Courier' size='8'>{code_text}</font>", code_style))
            self.story.append(Paragraph("[... 代码继续 ...]", self.styles['CustomBody']))
        self.story.append(PageBreak())
        ...
        if ref_file.exists():
            ...
            ref_style = ParagraphStyle(name='Refs', fontName='Courier', fontSize=8, leading=10)
            self.story.append(Paragraph(f"<font name='Courier' size='8'>{ref_text.replace('&', '&amp;').replace('<', '&lt;')}</font>", ref_style))
