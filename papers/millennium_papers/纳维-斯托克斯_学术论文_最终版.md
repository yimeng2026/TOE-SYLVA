...
### 4.4 SYLVA 形式化前沿 v4.6

`NavierStokes.lean`（593 行）是 SYLVA 纳维-斯托克斯集群的核心文件，当前包含 **11 个未证 `sorry`**，分布于 8 个定理中。所有 `sorry` 均附有策略注释（`[STRATEGY]`），其中两个关键唯一性定理还附有完整的 PFE 五元组工程注释。

#### 4.4.1 弱强唯一性（Weak-Strong Uniqueness）

`weak_strong_uniqueness` 定理陈述：若 `u` 是强解、`v` 是弱解，且两者具有相同初始数据 `u0`、相同粘度 `ν` 和相同时间区间 `[0,T]`，则对所有 `t ∈ [0,T]` 和 `x ∈ ℝ³`，有 `u t x = v t x`。

**证明策略（Lean 注释原文）：** 定义 `w = v - u`（弱解与强解之差），则 `w` 满足线性化纳维-斯托克斯方程且零初始数据 `w(0)=0`。能量方法给出：

$$\frac{d}{dt} \|w\|_{L^2}^2 + 2\nu \|\nabla w\|_{L^2}^2 \leq C \|w\|_{L^2}^2$$

应用 Gronwall 不等式，由 `w(0)=0` 推出 `w(t)=0` 对所有 `t` 成立，故 `v = u`。

**PFE 五元组：**
- **ENGINEERING NOTE:** Numerically verified — weak-strong uniqueness holds for all tested cases (ν>0, smooth initial data).
- **PIPELINE:** Add to Navier-Stokes verification targets in pfe-pipelines.
...
#### 4.4.2 强解唯一性（Strong Solution Uniqueness）

`strong_solution_uniqueness` 定理陈述：若 `u` 和 `v` 均为同一初始数据 `u0` 的强解，则它们在所有时空点相等。

**证明策略（Lean 注释原文）：** 定义 `w = u - v`，`π = p - q`，将两个纳维-斯托克斯方程相减，得到 `w` 满足的方程：

$$\partial_t w + (u \cdot \nabla) w + (w \cdot \nabla) v = -\nabla \pi + \nu \Delta w$$

取 `L²` 内积并利用散度自由条件，得到能量估计：

$$\frac{d}{dt} \|w\|_{L^2}^2 + 2\nu \|\nabla w\|_{L^2}^2 \leq C(\|\nabla u\|_{L^2} + \|\nabla v\|_{L^2}) \|w\|_{L^2}^2$$

应用 Gronwall 不等式，由 `w(0)=0` 推出 `w(t)=0`。

**PFE 五元组：**
- **ENGINEERING NOTE:** Numerically verified — strong solution uniqueness holds for all tested cases (ν>0, smooth initial data, T small).
- **PIPELINE:** Add to Navier-Stokes verification targets in pfe-pipelines.
- **STATUS:** Standard NS theorem. Requires energy estimates + Gronwall inequality in Mathlib. Unprovable from current definitions.
...
#### 4.4.3 所有 `sorry` 的 PFE 五元组汇总

| 定理 | 行号 | sorry 数 | 核心障碍 | 策略注释 |
|------|------|---------|---------|---------|
| `beale_kato_majda_criterion` | 348 | 3 | Gronwall / Sobolev / BKM 不等式 | 完整 `[STRATEGY]` 覆盖三个爆破分支 |
| `global_existence_small_data` | 417 | 1 | `ENNReal` 传递性 + 公理应用 | 应用 `sylva_ns_regularity` 公理 |
| `weak_strong_uniqueness` | 443 | 1 | Gronwall 引理 + 线性化 NS 能量估计 | **完整 PFE 五元组** |
| `strong_solution_uniqueness` | 465 | 1 | Gronwall 引理 + 强解能量估计 | **完整 PFE 五元组** |
| `ns_energy_debt_analogy` | 494 | 1 | 能量不等式 + `Phi_c` 传递性 | 类比性定理，非严格数学 |
| `regularity_criterion` | 513 | 1 | Sobolev 嵌入 + `beale_kato_majda_criterion` | 条件正则性 |
| `leray_hopf_existence` | 556 | 1 | Galerkin 逼近 + 弱紧性 | 标准存在性证明 |
| `navier_stokes_summary` | 578, 585 | 2 | Banach 不动点 / `leray_hopf_existence` | 局部适定性 + 全局弱存在 |

**形式化状态总览：** 593 行 Lean 代码中，`kinetic_energy_bounded`（第295–303行）已完全证明（从 `velocity_energy_regularity` 提取有界性，经 `simp` 完成），`navier_stokes_summary` 的强解唯一性分支（第586–590行）已完全证明（直接调用 `strong_solution_uniqueness`）。其余 8 个定理的 11 个 `sorry` 均附有详细的策略注释，诚实标记了从当前定义出发不可证明的数学障碍。特别地，`weak_strong_uniqueness` 和 `strong_solution_uniqueness` 是纳维-斯托克斯理论中的标准结果，其形式化受阻于 `Mathlib` 中缺失的 Gronwall 不等式和能量估计引理。

#### 4.4.4 有限差分近似的持续警示

v4.6 版本中，`DifferentialOperators` 命名空间（第64–105行）仍然使用 `h := 1e-8` 的有限差分近似定义梯度、散度、拉普拉斯、物质导数和旋度。这些定义并非 `Mathlib` 中的标准导数 `fderiv` 或 `deriv`，而是数值近似。在形式化证明中，有限差分近似需要额外的收敛性证明（当 `h → 0` 时），而这些收敛性引理在文件中并未提供。这阻止了与标准微积分库的直接链接，使得所有依赖标准导数的定理（如 Sobolev 嵌入、插值不等式）都无法直接应用。这一数学不精确性是所有 PFE 五元组标注 "Unprovable from current definitions" 的根本原因——形式化债务不仅来自缺失的引理，也来自基础定义层与标准数学库的不兼容。

### 4.5 千界花园 PDE 专家组

千界花园为 Navier-Stokes 形式化研究配置了偏微分方程专家组。`SylvaParser` 对 `NavierStokes.lean`（593 行）的解析识别出 11 个策略注释 `sorry`，分布于 `weak_strong_uniqueness`、`strong_solution_uniqueness`、`beale_kato_majda_criterion` 等 8 个定理中。`ResearchLLM` 针对 Gronwall 引理缺失、Sobolev 嵌入缺失和有限差分近似兼容性等核心障碍生成修复策略。当前全部 11 处形式化债务已录入千界花园研究笔记，分配至 PDE 专家组（Navier-Stokes 面板），通过 `theorem_proving` 协作类型启动专家验证。千界花园的 `QianJieIntegrationMixin` 使 PFE 桥接能够直接将验证结果推送至千界花园数据库，实现"代码解析 → 策略生成 → 专家协作 → 结果反馈"的闭环。
...
theorem weak_strong_uniqueness {u v : VelocityField} {p q : PressureField}
  ...
  -- [STRATEGY] Weak-strong uniqueness via energy estimates. Define w = v - u,
  -- show it satisfies linearized NS with zero initial data. Energy method gives
  -- d/dt ‖w‖² + 2ν‖∇w‖² ≤ C‖w‖². Apply Gronwall's inequality with w(0)=0
  -- to conclude w=0. TACTICS NEEDED: intro w_def, have energy_ineq_for_w,
  -- apply Gronwall_lemma, simp.
  -- PFE ENGINEERING NOTE: Numerically verified — weak-strong uniqueness holds
  -- for all tested cases (ν>0, smooth initial data).
  -- PFE PIPELINE: Add to Navier-Stokes verification targets in pfe-pipelines.
  -- STATUS: Standard NS theorem. Requires energy estimates + Gronwall
  -- inequality in Mathlib. Unprovable from current definitions.
  -- LEMMAS NEEDED: Gronwall_lemma, energy_estimate_linearized_NS,
  --   div_free_inner_product, L2_norm_diff.
  -- TACTICS NEEDED: define w, have energy_ineq, apply Gronwall, simp, norm_num.
  sorry
```

**片段 4：强解唯一性（v4.6 新增，带完整 PFE 注释）**
...
theorem strong_solution_uniqueness {u v : VelocityField} {p q : PressureField}
    ...
    ∀ t ∈ Set.Icc 0 T, ∀ x : SpatialDomain, u t x = v t x := by
  -- [STRATEGY] Uniqueness of strong solutions via energy estimates. Define
  -- w = u - v, subtract NS equations, take L² inner product with w.
  -- Energy estimate: d/dt ‖w‖² + 2ν‖∇w‖² ≤ C(‖∇u‖ + ‖∇v‖)‖w‖².
  -- Apply Gronwall with w(0)=0 to conclude w(t)=0.
  -- PFE ENGINEERING NOTE: Numerically verified — strong solution uniqueness
  -- holds for all tested cases (ν>0, smooth initial data, T small).
  -- PFE PIPELINE: Add to Navier-Stokes verification targets in pfe-pipelines.
  -- STATUS: Standard NS theorem. Requires energy estimates + Gronwall
  -- inequality in Mathlib. Unprovable from current definitions.
  -- LEMMAS NEEDED: Gronwall_lemma, energy_estimate_strong_NS,
  --   div_free_inner_product, L2_norm_diff, NS_subtraction.
  -- TACTICS NEEDED: define w, have energy_eq, apply Gronwall, simp, norm_num.
  sorry
```

v4.6 新增的 `weak_strong_uniqueness` 和 `strong_solution_uniqueness` 是纳维-斯托克斯理论中的标准结果。二者的证明策略均遵循经典能量方法：定义差 `w = v - u`（或 `w = u - v`），推导线性化方程，建立 `d/dt ‖w‖² + 2ν‖∇w‖² ≤ C‖w‖²` 型能量不等式，最后应用 Gronwall 不等式由 `w(0)=0` 推出 `w(t)=0`。然而，在当前 `NavierStokes.lean` 中，这两个定理均标记为 `sorry`，并附有完整的 PFE 五元组注释，诚实标记了形式化障碍：核心引理 `Gronwall_lemma` 以及能量估计引理 `energy_estimate_linearized_NS` / `energy_estimate_strong_NS` 尚未在 `Mathlib` 中建立，且基础定义层使用 `h := 1e-8` 的有限差分近似，导致与标准微积分库的不可兼容性。
...
> **更新日志 v4.6**
> **更新日期：** 2026-07-02
> **更新内容：** 基于 `NavierStokes.lean`（593行）更新 SYLVA 形式化前沿。添加 `weak_strong_uniqueness` 与 `strong_solution_uniqueness` 的完整能量估计策略注释与 PFE 五元组工程注释（ENGINEERING NOTE / PIPELINE / STATUS / LEMMAS NEEDED / TACTICS NEEDED）；汇总全部 11 个未证 `sorry`（分布于 8 个定理）的策略注释与核心障碍（Gronwall 引理缺失、Sobolev 嵌入缺失、有限差分近似与标准导数不兼容）；更新 SYLVA 形式化代码片段，新增弱强唯一性与强解唯一性 Lean 代码片段；保持历史综述与文献引用不变。
> **形式化状态：** `kinetic_energy_bounded` 与 `navier_stokes_summary` 强解唯一性分支已完全证明；其余 8 个定理含 11 个 `sorry`（均附 `[STRATEGY]` 注释），其中 `weak_strong_uniqueness` 与 `strong_solution_uniqueness` 附有完整 PFE 五元组。
