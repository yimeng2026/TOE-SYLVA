...
> **📌 Version Note.** This is the **v7.0 final edition** of the Counting Geometry Survey. > **⚠️ Correction (2026-08-01).** Both `COUNTING_GEOMETRY_SURVEY.md` and `COUNTING_GEOMETRY_HONEST_FINAL.md` now contain **v7.0** content (the post-audit state). The true **v6.0** pre-audit stable version is available only in git history, at commit `96243d06`. If you need the exact pre-audit text, check out that commit: `git show 96243d06:COUNTING_GEOMETRY_SURVEY.md`. The v7.0 edition incorporates all corrections from the AI hallucination audit (see `AI_HALLUCINATION_REPORT_FINAL.md`), adds explicit "Open Problem" markers, and removes all unverifiable claims. Readers who want the pre-audit state of the survey should consult the v6.0 stable version.

## Revision History

| Version | Date | Changes |
|---------|------|--------|
| v6.0 | 2026-07 | Stable version (pre-audit). Preserved in `COUNTING_GEOMETRY_SURVEY.md`. Contains structural strengths but also fabricated formulas, fabricated citations, and inflated claims. |
| v7.0 | 2026-08 | Final honest edition (this file). All 15 fabricated claims deleted (see `data/deleted_fabricated_claims.json`). 15+ citations corrected. MNOP proof status updated to include Pardon 2023 (arXiv:2308.02948, Fields Medal 2026). All claims anchored to stable identifiers. |

**Key differences v6.0 → v7.0:**
- Termwise MNOP formula deleted; replaced with honest partition-function statement.
- "Universal GW closed formula" deleted; replaced with correctly attributed Faber–Pandharipande and Bouchard et al. formulas.
- Mirror-symmetry termwise formula deleted; replaced with Batyrev 1994 and honest open problems.
- Pardon 2023 MNOP proof added with correct scope (nef anticanonical, primary insertions only).
- Ngô citation corrected (*Publ. Math. IHÉS* 111, 2010, not *Annals* 181(2), 2015).
- All Lean 4 / Mathlib PR claims withdrawn.
- "Virtual stack axiom" and all derivative "theorems" deleted.
- Yau–Zaslow formula corrected to include divisibility index r; non-primitive cases marked as open.
- Borcea–Voisin char-p caveat added (fails for p > 2).
...
**Gromov's pseudoholomorphic curves (1985).** Gromov introduced pseudoholomorphic curves into symplectic geometry in the landmark paper *"Pseudo holomorphic curves in symplectic manifolds"*, Inventiones Mathematicae 82(2), 1985, pp. 307–347; Zbl 0592.53025 — the seed of Gromov–Witten theory and the subject of his 1997 AMS Steele Prize. The paper established the compactness and existence theory for J-holomorphic curves that later underpinned Gromov–Witten invariants.
...
**Donaldson–Thomas theory (1998).** Donaldson and Thomas proposed counting ideal sheaves on Calabi–Yau threefolds as a holomorphic analogue of Donaldson invariants in four dimensions (*Gauge theory in higher dimensions*, in *The Geometric Universe* (S. A. Huggett et al., eds.), Oxford University Press, 1998, pp. 31–47; Zbl 0926.58003). The resulting DT invariants, defined via the Euler characteristic of Hilbert schemes weighted by the Behrend function, initially seemed completely different from GW invariants. Thomas later developed the rigorous virtual-cycle foundation in *"A holomorphic Casson invariant for Calabi–Yau 3-folds, and bundles on K3 fibrations"*, Journal of Differential Geometry 54(2), 2000, pp. 367–438; Zbl 1033.14012.
...
which, up to the convention for the q‑shift, is the reciprocal of the modular discriminant Δ(τ) = q ∏(1−q^m)^{24}. The exponent 24 is the Euler characteristic of a K3 surface. The case r = 1 (primitive classes) is the one most often stated; the general r version is the full conjecture and is **not** a theorem in full generality.

### 5.2 What earlier drafts got wrong

Earlier versions stated $N_{0,d}(\mathrm{K3}) = 24\cdot\sigma(d)$ as if it were the general formula. This is only the **r = 1 (primitive) special case** in the notation of Göttsche's lectures, not the full generating‑function statement. The honest, general statement is the product formula above, and even for non‑primitive classes (r > 1) the conjecture was only verified case‑by‑case (e.g. Lee–Leung for r = 2) — it is **not** proved in full generality.
...
- Batyrev, *Dual polyhedra and mirror symmetry for Calabi–Yau hypersurfaces in toric varieties*, Journal of Algebraic Geometry 3(3), 1994, pp. 493–535; Zbl 0829.14023.
- Borisov, *Vertex algebras and mirror symmetry*, Communications in Mathematical Physics 215(3), 2001, pp. 517–557; Zbl 0990.17023 — algebraic approach to toric mirror symmetry via sheaves of vertex algebras. (Borisov also studied mirror duality for Gorenstein toric Fano varieties in the preprint math.AG/9310001.)

### 6.3 Toric mirror construction (Batyrev)

**Source:** Batyrev, *Dual polyhedra and mirror symmetry for Calabi–Yau hypersurfaces in toric varieties*, Journal of Algebraic Geometry 3(3), 1994, pp. 493–535; Zbl 0829.14023.
...
This was generalized by **Batyrev–Borisov** to complete intersections in toric varieties using nef partitions and Gorenstein cones: *Dual cones and mirror symmetry for generalized Calabi–Yau manifolds*, in *Mirror Symmetry II*, AMS/IP Stud. Adv. Math. 1, Amer. Math. Soc., 1997, pp. 71–86; and *On Calabi–Yau complete intersections in toric varieties*, in *Higher-dimensional complex varieties* (Trento, 1994), de Gruyter, 1996, pp. 39–65.
...
**Source:** Donaldson–Thomas, *Gauge theory in higher dimensions*, in *The Geometric Universe*, Oxford University Press, 1998, pp. 31–47; Zbl 0926.58003. Thomas, *A holomorphic Casson invariant for Calabi–Yau 3-folds, and bundles on K3 fibrations*, Journal of Differential Geometry 54(2), 2000, pp. 367–438; Zbl 1033.14012. (arXiv:math/9806111)
...
- **Ngô Bao Châu's proof of the fundamental lemma** (*Le lemme fondamental pour les algèbres de Lie*, Publications Mathématiques de l'IHÉS **111**(2010), pp. 1–169; arXiv:0801.0446 (2008)) — a landmark result using geometric methods (Hitchin fibration, support theorem). *(Corrected 2026-08; previously mis-cited as Annals 181(2), 2015.)*
...
> **Note:** `data/` and `scripts/` are sibling directories at the repository root. Both files (`data/deleted_fabricated_claims.json` and `scripts/verify_honest_repo.py`) exist and are accessible at the repository top level.
