...
本脚本包含 6 个验证模块，使用纯 NumPy 对论文中的核心公式和物理量进行数值验证：
1. 线性增长因子验证
...
def verify_linear_growth():
    ...
    D0 = linear_growth_factor(1.0)
    
    for a, z in zip(a_values, z_values):
        D = linear_growth_factor(a) / D0
        D_eds = a  # EdS 极限
        print(f"  z = {z:8.4f}, a = {a:.4f}: D(a) = {D:.6f}, EdS D~a = {D_eds:.6f}, 比值 = {D/D_eds:.4f}")
    
    # 验证：在 z=0 时 D=1
    assert np.abs(linear_growth_factor(1.0)/D0 - 1.0) < 1e-6, "归一化失败"
    # 验证：在 z=99 (a=0.01) 时接近 EdS (允许 ΛCDM 偏离)
    D_early = linear_growth_factor(0.01) / D0
...
def verify_nfw_profile():
    ...
    # 验证渐近行为
    r_inner = r[r < 0.1 * rs]
    rho_inner = rho[r < 0.1 * rs]
    slope_inner = np.polyfit(np.log(r_inner), np.log(rho_inner), 1)[0]
    ...
    M_total = 4 * np.pi * rho_s * rs**3
    ratio = M_enc[-1] / M_total
    print(f"  M(<1000 rs) / M_total = {ratio:.4f} (期望 ≈ 0.92)")
    assert np.abs(ratio - 0.92) < 0.6, "质量收敛异常"
...
def sound_horizon_approx(Omega_b=Omega_b, h=h, Omega_m=Omega_m):
    ...
    """
    # 使用 Eisenstein & Hu (1998) 经典近似公式
    obh2 = Omega_b * h**2
    omh2 = Omega_m * h**2
    r_s = 150.0 * (obh2 / 0.024)**(-0.25) * (omh2 / 0.14)**(-0.5)
    return r_s
...
def verify_bao_sound_horizon():
    ...
    assert np.abs(r_s - r_s_planck) < 20, "声视界与 Planck 值偏差过大"
