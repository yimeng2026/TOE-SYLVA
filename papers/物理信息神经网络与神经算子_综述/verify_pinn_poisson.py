...
# -*- coding: utf-8 -*-
"""
verify_pinn_poisson.py
======================
物理信息神经网络 (PINN) 与神经算子综述 §8 数值验证脚本。
纯 numpy + scipy 实现 (无 PyTorch / JAX 依赖), 运行 < 30 s.

模块
----
M1  PINN 解 2D Poisson 方程: -Δu = 2π² sin(πx) sin(πy), u|∂Ω = 0 on [0,1]²
    解析解 u = sin(πx) sin(πy). MLP(2-25-25-1, tanh). 
    Laplacian 用 5 点中心差分在解析 forward-mode 一阶导数基础上推出.
    L-BFGS-B (解析梯度) 优化, 与解析解相对 L² 误差 < 5%.
M2  谱偏置检验: MLP(1-30-30-30-1) 拟合 sin(2πx) + 0.5·sin(10πx).
    训练 5000 步 Adam, 跟踪低/高频幅度. 阈值: iter=1000 时 
    低频相对误差严格小于高频相对误差 (err_low/err_high < 0.85).
M3  Fourier 特征克服谱偏置: 对照 vanilla MLP 与 Random Fourier Feature
    (RFF) 线性模型在高频目标 sin(20πx) 上的精度.
    阈值: RFF 相对误差 < vanilla MLP 的 1/3.

作者: 并行搜索员 A (id 6a6167e63b927fe24be17ea1)
日期: 2026-07-26
依据: 全面推进策略_2026-07-26.md 第一部分 A7 小节
"""
...
def mlp_backward(params, acts, zs, grad_output):
    ...
    返回 grad_params: [[dW, db], ...] (与 params 同结构).
    注: grad_output 已包含 1/N 因子 (若 L = mean over batch), 
    grad_W = grad_z.T @ a_prev 不再除以 N.
    """
    L = len(params)
    grad_params = [None] * L
    grad_a = grad_output
    for i in range(L - 1, -1, -1):
        ...
        grad_W = grad_z.T @ a_prev       # (d_out, d_in)
        grad_b = grad_z.sum(axis=0)      # (d_out,)
        grad_params[i] = [grad_W, grad_b]
        grad_a = grad_z @ W              # (N, d_{i-1})
    return grad_params
...
def make_collocation_bc(N_col=256, N_bc_per_side=24, seed=SEED):
    """生成 [0,1]² 上的 collocation 与 boundary 点. u=0 on ∂Ω."""
    r = np.random.default_rng(seed)
    # Sobol-like jitter on uniform grid
...
def laplacian_via_fd(params, xy, h=1e-3):
    """5 点中心差分计算 Laplacian Δu = ∂²u/∂x² + ∂²u/∂y².
    返回 lap: (N, 1)."""
    N = xy.shape[0]
    x, y = xy[:, 0], xy[:, 1]
    pts = np.vstack([
        np.stack([x + h, y], axis=1),
        np.stack([x - h, y], axis=1),
        np.stack([x, y + h], axis=1),
        np.stack([x, y - h], axis=1),
        np.stack([x, y], axis=1),
    ])
    u_all, _, _ = mlp_forward(params, pts)
    u_all = u_all.reshape(5, N)
    u_xph, u_xmh, u_yph, u_ymh, u_0 = u_all
    lap = ((u_xph - 2 * u_0 + u_xmh) / h ** 2
           + (u_yph - 2 * u_0 + u_ymh) / h ** 2)
    return lap.reshape(-1, 1)


def loss_and_grad_poisson(params_flat, dims, xy_col, xy_bc, u_bc,
                          f_col, h_fd=1e-3, lam_bc=20.0):
    """
    强形式 Poisson PINN 损失:
        L = (1/N_r) Σ |r_i|² + λ_b · (1/N_b) Σ |u(x_b) - u_b|²
        其中 r = -Δu - f (Δu 通过 5 点中心差分).
    解析梯度: 通过对 5 个差分模板点各做一次反向传播再线性组合得到.
    """
    ...
    u_xph, a_xph, z_xph = mlp_forward(params, pts_xph)
    u_xmh, a_xmh, z_xmh = mlp_forward(params, pts_xmh)
    u_yph, a_yph, z_yph = mlp_forward(params, pts_yph)
    u_ymh, a_ymh, z_ymh = mlp_forward(params, pts_ymh)
    u_0, a_0, z_0 = mlp_forward(params, xy_col)

    # Laplacian via FD
    lap = ((u_xph - 2 * u_0 + u_xmh) / h_fd ** 2
           + (u_yph - 2 * u_0 + u_ymh) / h_fd ** 2)  # (N_c, 1)
    ...
    # Boundary
    u_bc_pred, a_bc, z_bc = mlp_forward(params, xy_bc)
    bc_err = u_bc_pred - u_bc  # (N_b, 1)
    L_bc = float(np.mean(bc_err ** 2))
    L_total = L_pde + lam_bc * L_bc

    # ---------- 梯度 ----------
    # dL/dr = 2r / N_c
    grad_r = 2.0 * r / N_c                       # (N_c, 1)
    # dr/d(lap) = -1  →  dL/d(lap) = -grad_r
    grad_lap = -grad_r                           # (N_c, 1)
    # 模板导数: d(lap)/d(u_xph) = 1/h²,  d(lap)/d(u_0) = -4/h², ...
    inv_h2 = 1.0 / h_fd ** 2
    ...
    # 反向传播每个模板点
    gp_xph = mlp_backward(params, a_xph, z_xph, grad_u_xph)
    gp_xmh = mlp_backward(params, a_xmh, z_xmh, grad_u_xmh)
    gp_yph = mlp_backward(params, a_yph, z_yph, grad_u_yph)
    gp_ymh = mlp_backward(params, a_ymh, z_ymh, grad_u_ymh)
    gp_0 = mlp_backward(params, a_0, z_0, grad_u_0)
    ...
    for i in range(len(params)):
        ...
        db = (gp_xph[i][1] + gp_xmh[i][1] + gp_yph[i][1]
              + gp_ymh[i][1] + gp_0[i][1])
        grad_params_pde.append([dW, db])

    # Boundary gradient
    grad_u_bc = 2.0 * lam_bc * bc_err / N_b
    gp_bc = mlp_backward(params, a_bc, z_bc, grad_u_bc)

    grad_params_total = []
    for i in range(len(params)):
        dW = grad_params_pde[i][0] + gp_bc[i][0]
        db = grad_params_pde[i][1] + gp_bc[i][1]
        grad_params_total.append([dW, db])

    grad_flat = flatten_params(grad_params_total)
    return L_total, grad_flat


def hard_constraint_u(params, xy):
    """硬约束: u_pred = x(1-x)y(1-y) · NN(x,y). 自动满足 u|∂Ω=0.
    返回 (u_pred, acts, zs) — 对 NN 部分的前向结果."""
    x = xy[:, 0:1]
    y = xy[:, 1:2]
    g = x * (1 - x) * y * (1 - y)  # 硬约束因子, (N, 1)
    nn_out, acts, zs = mlp_forward(params, xy)
    u_pred = g * nn_out
    return u_pred, acts, zs, g


def laplacian_hard_constraint(params, xy, h=1e-3):
    """5 点 FD 计算 u_pred 的 Laplacian (硬约束形式)."""
    N = xy.shape[0]
    x, y = xy[:, 0], xy[:, 1]
    pts = np.vstack([
        np.stack([x + h, y], axis=1),
    ...
    u_all, _, _, _ = hard_constraint_u(params, pts)
    u_all = u_all.reshape(5, N)
    u_xph, u_xmh, u_yph, u_ymh, u_0 = u_all
    lap = ((u_xph - 2 * u_0 + u_xmh) / h ** 2
           + (u_yph - 2 * u_0 + u_ymh) / h ** 2)
    return lap.reshape(-1, 1), u_0


def loss_and_grad_hard(params_flat, dims, xy_col, f_col, h_fd=1e-3):
    """硬约束 PINN: L = (1/N) Σ |r|², r = -Δu - f. u=x(1-x)y(1-y)·NN."""
    params = unflatten_params(params_flat, dims)
    N_c = xy_col.shape[0]
    x_c, y_c = xy_col[:, 0], xy_col[:, 1]
    ...
    # 由于 u = g · NN, dL/d(NN_output) = dL/d(u) · g
...
def adam_optimize(grad_fn, params_flat_0, n_iters=4000, lr=1e-3,
    ...
    """Adam 优化器. grad_fn(params_flat) -> (loss, grad)."""
...
def module_M1(out_dir):
    print("\n[M1] PINN 解 2D Poisson: -Δu = 2π² sin(πx) sin(πy), u|∂Ω=0")
    print("-" * 60)
    print("  (硬约束 u=x(1-x)y(1-y)·NN, 自动满足 Dirichlet BC)")

    dims = DIMS_M1
    xy_col, _, _ = make_collocation_bc(N_col=256, N_bc_per_side=24,
                                       seed=SEED)
    f_col = (2.0 * np.pi ** 2
             * np.sin(np.pi * xy_col[:, 0:1])
             * np.sin(np.pi * xy_col[:, 1:2]))

    # 初始化网络 (较小权重, 利于 PINN 收敛)
    params0 = init_mlp(dims, seed=SEED)
    params0 = [[W * 0.5, b] for W, b in params0]  # 缩小初始权重
    ...
    print(f"  相对 L² 误差 = {rel_l2:.4e}  (阈值 < 5e-2)")
    ...
    im2 = axes[2].pcolormesh(XX, YY, err, shading='auto', cmap='RdBu',
                              vmin=-vmax, vmax=vmax)
    axes[2].set_title("误差分布 $u_{\\rm pred}-u_{\\rm true}$")
    axes[2].set_xlabel("$x$"); axes[2].set_ylabel("$y$")
    fig.colorbar(im2, ax=axes[2])
    fig.suptitle(f"[M1] PINN 解 2D Poisson 方程 (MLP {dims}, $L^{{ rel}}_2$"
                 f"={rel_l2:.2e})", y=1.03)
    fig.tight_layout()
...
def mlp_forward_1d(params, x):
    """1D 输入 MLP 前向, 返回 (u, acts, zs)."""
    return mlp_forward(params, x)  # 复用通用前向


def compute_amplitudes(x_grid, pred):
    """用最小二乘拟合 pred ≈ a₁ sin(2πx) + b₁ cos(2πx)
                              + a₂ sin(10πx) + b₂ cos(10πx),
    返回 (amp_low, amp_high)."""
    A = np.column_stack([
...
def module_M2(out_dir):
    print("\n[M2] 谱偏置: MLP 拟合 sin(2πx) + 0.5·sin(10πx)")
    print("-" * 60)
    dims = DIMS_M2
    ...
    for it in range(1, n_iters + 1):
        # 前向
        y_pred, acts, zs = mlp_forward_1d(params, x_train)
        err = y_pred - y_train  # (N, 1)
        ...
        if it in snapshot_iters:
            y_test_pred, _, _ = mlp_forward_1d(params, x_test)
    ...
    # 阈值: iter=1000 时 err_low/err_high < 0.85 (低频显著快于高频)
    check_iter = 1000
    ...
    y_final, _, _ = mlp_forward_1d(params, x_test)
...
def rff_features(x, B, sigma=1.0):
    """Random Fourier Feature: φ(x) = [cos(2π B x), sin(2π B x)] / sqrt(2K)."""
    # x: (N, 1), B: (K,) random frequencies
    arg = 2 * np.pi * (x * B[None, :])  # (N, K)
    feats = np.concatenate([np.cos(arg), np.sin(arg)], axis=1)  # (N, 2K)
    return feats / np.sqrt(2 * B.shape[0])


def module_M3(out_dir):
    """对照 vanilla MLP 与 Random Fourier Feature 线性模型."""
    print("\n[M3] Fourier 特征克服谱偏置 (vanilla MLP vs RFF)")
    print("-" * 60)

    def target(x):
        return np.sin(20 * np.pi * x)  # 10 Hz 高频
    ...
    print(f"  vanilla MLP ({dims}), {n_iters} Adam 步, "
          f"耗时 {t_vanilla:.2f} s")
    print(f"    rel L² = {rel_l2_v:.4e}, 高频幅度 = {amp_v:.4f} (目标 1.0)")

    # ---------- RFF 线性模型 (闭式) ----------
    rng = np.random.default_rng(SEED + 1)
    sigma_rff = 8.0  # 频率尺度, 匹配目标频率 (10 Hz 量级)
    K_rff = 128
    B = rng.normal(0, sigma_rff, size=K_rff)  # 随机频率
    ...
    print(f"  RFF 线性模型 (K={K_rff}, σ={sigma_rff}), "
          f"rel L² = {rel_l2_rff:.4e}, 高频幅度 = {amp_rff:.4f}")

    # 阈值: RFF 误差 < vanilla MLP 误差的 1/3
    threshold_ratio = 3.0
    ...
    axes[1].set_title(f"误差对比 (vanilla/RFF = {ratio:.2f}×)")
...
def main():
    ...
    print("=" * 70)
    print("verify_pinn_poisson.py | 物理信息神经网络与神经算子综述 §8 验证")
    print("=" * 70)
