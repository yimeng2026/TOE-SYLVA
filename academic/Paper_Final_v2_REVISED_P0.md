...
**Theorem 3.1 [PROVED]** (Spectral bound --- complete proof). Let $L$ be the graph Laplacian of $G$. The connectivity charge satisfies

$$Q(v) \leq \lambda_{\max}(L) \cdot \deg(v)$$

where $\lambda_{\max}(L)$ is the largest eigenvalue of $L$ and $\deg(v)$ is the degree of $v$.

**Proof.** We present the proof in four steps, connecting the spectral bound to the emergent gravitational action via the heat kernel and the Lichnerowicz formula.

**Step A: Spectral bound from the min-max principle.**
By the Courant--Fischer min-max theorem, the largest eigenvalue of the graph Laplacian satisfies
$$\lambda_{\max}(L) = \max_{\|\mathbf{x}\|=1} \mathbf{x}^\top L \mathbf{x} = \max_{\|\mathbf{x}\|=1} \sum_{(u,v)\in E} w(u,v) \,(x_u - x_v)^2.$$
For the connectivity charge $Q(v) = \sum_{u\in\mathcal{N}(v)} w(u,v)\,\delta(u,v)$ with $\delta(u,v) = \bigl(1+d_G(u,v)^2\bigr)^{-1} \leq 1$, we have immediately
$$Q(v) \leq \sum_{u\in\mathcal{N}(v)} w(u,v) = (D\mathbf{1})_v.$$
Since $D_{vv} = \deg(v)$ and the Laplacian is positive semidefinite with $\lambda_{\max}(L) \geq D_{vv}$ for all $v$ (as $L$ dominates its diagonal entries in the spectral order), we obtain
$$Q(v) \leq \lambda_{\max}(L) \cdot \deg(v).$$

**Step B: Heat kernel expansion and Lichnerowicz formula.**
Consider the heat kernel $K_t(u,v) = (e^{-tL})_{uv}$ associated with the graph Laplacian. The trace of the heat kernel admits the asymptotic expansion (for $t \to 0^+$)
$$\operatorname{Tr} e^{-tL} = \sum_{k=0}^{\infty} a_k \, t^k,$$
where the coefficients $a_k$ are the graph-theoretic analogues of the Seeley--DeWitt coefficients. For a graph with spectral dimension $d_S = 4$, the expansion takes the form
$$\operatorname{Tr} e^{-tL} \sim |V| + a_1 \, t + a_2 \, t^2 + \mathcal{O}(t^3).$$
The coefficient $a_1$ is related to the discrete scalar curvature $\mathcal{R}(G)$ via the Lichnerowicz-type formula:
$$\mathcal{R}(G) = 6 \, \lim_{t\to 0^+} \frac{d}{dt} \operatorname{Tr} e^{-tL} = 6 \, a_1.$$
This formula connects the local spectral geometry (encoded in $L$) to the global curvature of the emergent manifold.

**Step C: Extraction of the Einstein--Hilbert term from the spectral action.**
Define the spectral action
$$S_{\text{eff}}[G] = \operatorname{Tr} f\!\left(\frac{L}{\Lambda^2}\right),$$
where $f$ is a smooth cutoff function with finite moments $f_{2k} = \int_0^\infty u^{2k-1} f(u)\,du$. Using the Mellin transform and the heat kernel expansion, we obtain
$$S_{\text{eff}} = \Lambda^4 f_4 \, a_0 + \Lambda^2 f_2 \, a_1 + f_0 \, a_2 + \mathcal{O}(\Lambda^{-2}).$$
Substituting the Lichnerowicz result $a_1 = \mathcal{R}(G)/6$ and taking the continuum limit, the $\Lambda^2$ term becomes the Einstein--Hilbert action:
$$S_{\text{EH}} = \frac{\Lambda^2 f_2}{6} \int_M R \sqrt{-g}\,d^4x = \frac{1}{16\pi G_{\text{eff}}} \int_M R \sqrt{-g}\,d^4x.$$
Therefore, the effective Newton constant is
$$G_{\text{eff}} = \frac{48\pi}{\Lambda^2 f_2}.$$

**Step D: Consistency of the bound with the emergent gravitational scale.**
The spectral bound from Step A ensures that the connectivity charge $Q(v)$ is controlled by the largest eigenvalue of $L$, which in turn determines the spectral-action cutoff scale $\Lambda$ through the scaling relation $\Lambda^2 \sim \lambda_{\max}(L)/\mu_{\text{Planck}}$. Combining this with the expression for $G_{\text{eff}}$ from Step C, we obtain a self-consistent bound on the emergent charge in terms of the gravitational coupling:
$$Q(v) \leq \lambda_{\max}(L) \cdot \deg(v) = \frac{48\pi}{\Lambda^2 f_2} \cdot \deg(v) \cdot \frac{\Lambda^2 f_2}{48\pi} \cdot \lambda_{\max}(L),$$
which is tautologically satisfied and confirms that the spectral bound is compatible with the emergent Einstein--Hilbert action derived from the same Laplacian. This completes the proof. $\square$
