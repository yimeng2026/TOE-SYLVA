...
> **日期**: 2026-08-21
> **HEAD**: v7.97
> **任务来源**: 群管理员 §三新方向规划；策略依据为 `DEVELOPMENT_DIRECTIONS.md` §三「五大新发展方向」
...
## 一、现状基线（2026-08-21，HEAD = v7.97）

本路线图的所有结合点均锚定以下实测基线：

1. **公理治理**：`framework/axiom_registry.json` v2.0（2026-08-19）登记 **448 条** axiom（primitive 250 / definitional 142 / placeholder 42 / schema 14；其中可清偿评估 reducible 198 条，44.2%），覆盖 116 个非批量 Lean 文件。
2. **清偿成果**：累计 **136 条** axiom→theorem/def 清偿（sweep1 30 条 + sweep2 44 条 + sweep3 31 条 + sweep4 29 条，另含 v7.94 归档文件清偿；四轮报告见 `framework/axiom_reduction_sweep1-4_report.md`）。sweep5 正在并行执行（definitional 剩余量约 42–61 条，口径差异见 sweep3/sweep4 报告注记），本轮路线图按"sweep5 完成后 definitional 接近清零、primitive 成为主战场"预估。
3. **THEOREM 登记**：`framework/proof_status.md` §三现有 **3 项** THEOREM（CHSH 违例、Dedekind 整环理想唯一分解 `ideal_uniqueFactorizationMonoid`、StratifiedChernNumber T3 整数权整性定理——2026-08-21 本日登记，Chern-Simons 链升级完成，主链 L0–L5 与 8 个侧链已清点，剩余缺口四项详见 `framework/chern_simons_theorem_report.md`）。
...
7. **CI**：`python3 scripts/verify_honest_repo.py --ci` 当前 PASS；`lake build` 仍受约 36 万 .lean 文件拖累。CI 瘦身 §5.2 **phase1 已完成**（2026-08-21：生成器可复现性验证通过，5 模块迁移试点落地，详见 `framework/ci_slimming_phase1_report.md`）；另 phase1 sorry-audit 实测批量模块 30.2% 含 sorry（见 A-2）。
...
| A-2 | TrivialBench v0.1 采样与标注规范：从 28.8 万批量模块采样 5,000–10,000 条，按 `trivial`/`rfl`/`add_zero`/`¬False.elim` 等模式类别标注；**采样协议须先过滤含 `sorry` 的模块**（CI 瘦身 phase1 实测：git 索引内 119,859 个 SYLVA_Proven* 文件中 36,199 个含 sorry，占 30.2%，污染率不可忽略） | 数据卡 + 采样规范文档 + 标注样本 ≥1,000 条 | 抽样复检标注一致率 ≥95%；基准语料零 sorry 污染 |
...
| A-4 | 公理清偿自动化对接：以 axiom_registry v2.0 剩余 definitional 条目为测试集，验证 LLM 建议 + 人工确认的半自动清偿流程 | 半自动清偿试点报告 | 对 sweep5 之后剩余条目给出可执行建议 ≥20 条 |
...
1. `lake build` 可用（**关键路径**：依赖 CI 瘦身 §5.2 完成；phase1 试点进行中）；
2. Lean 4 + Mathlib 环境与 LLM API 访问；
3. RLMEval 研究级基准选型确认（不以 curated 基准自报成绩，遵守 `DEVELOPMENT_DIRECTIONS.md` §4.3 纪律）。

### A.4 与现有框架的结合点

- **448 条公理登记表**是现成的"公理→定理清偿建议"自动形式化测试集（人工 sweep1–5 的 136 条清偿记录可作为训练/评估对照样本）；
...
- 136 条清偿记录构成时变超边（axiom→theorem 的状态迁移历史），是"理论网络演化"的首批真实样本；
...
|----------|------|-----------|:------:|----------|
| ① 公理审计与诚实证明率治理方法论（proof_status + axiom_registry v2.0 + sweep1–4 共 136 条清偿 + verify_honest_repo.py + 口径清算史） | 文档链完整、数据全真、故事自洽 | 形式化方法期刊 | ★★★★★ | 需要按论文体裁重写；与 mathlib 社区定位对话 |
| ② Chern-Simons 不变量链 + Dedekind 课程链形式化案例（CLAIM→THEOREM 治理路径） | **T3 已于 2026-08-21 登记为第三项 THEOREM**（真实编译零错误零 sorry，#print axioms 仅标准三件套），链式状态与剩余缺口已清点 | 数学物理 × 形式化交叉 | ★★★★★（升级后） | mathlib 委托证明的原创性表述需谨慎；T3′/T6/T7 三条补强待独立编译复核 |
...
**推荐主线**：候选 ① 为第一优先（素材全真、无新数学风险、对 Lean 社区有元研究价值、"唯一真正的新贡献"认定背书），候选 ② 为第二优先——**本轮 CS 链升级已落定（T3 登记），② 的故事已完整**，与 ① 可组成"方法论 + 案例"双件套投递不同期刊。候选 ③/④ 中 qec_ftqc 可作为观察项，待 ①/② 打样流程成熟后复用。

### D.3 候选期刊清单

> 审稿周期与费用为大致量级，**投稿前以期刊官网为准**；预印本政策需逐刊核对（下列主要期刊均允许 arXiv 预印本）。

**D.3.1 形式化方法方向（候选 ① 的主战场）**

| 期刊/会议 | 出版方 | 定位匹配 | 备注 |
|-----------|--------|----------|------|
| Journal of Automated Reasoning (JAR) | Springer | 大型证明库治理、公理审计、自动化证明经验 | 综合性期刊，接受"经验+方法论"类论文，首选 |
| Logical Methods in Computer Science (LMCS) | 开放获取 | 形式化方法 + 计算逻辑交叉 | 免版面费；审稿相对快 |
| Journal of Formalized Reasoning (JFR) | 自由 OA | 形式化经验论文、mathlib 生态实践 | 体量小而对口 |
| Mathematics in Computer Science (MCS) | Springer | 数学形式化、计算机辅助数学 | 交叉定位合适 |
| 会议路线：CPP（Certified Programs and Proofs）/ ITP（Interactive Theorem Proving） | ACM/会议 | Lean 生态论文常见去向 | 有截稿日约束，作备选通道 |

**D.3.2 数学物理方向（候选 ②③④ 的主战场）**
...
| Letters in Mathematical Physics (LMP) | Springer | 短文快通道 | 适合 CS 链单点突破的 Letter |
...
| 素材 | 第一匹配 | 备选 |
|------|----------|------|
| ① 方法论 | JAR | LMCS → CPP/ITP 会议 |
| ② CS 链案例 | LMP（Letter）或 JMP | J. Phys. A → CMP（长线） |
...
2. **同行级重审**：全部候选素材未经独立同行评审，需按"模拟审稿人"流程过一遍（重点：数字口径一致性、引用-论述语义一致性）；
...
8. **审稿回复预案**：预判三类质疑——"AI 生成内容的可靠性""448 条公理的非标准性""批量模块对结论的污染"，逐条准备证据式回复（大量素材已在 DEVELOPMENT_DIRECTIONS §1 预置）；
...
| 阶段 | 周次 | 内容 | 关键产出 |
|------|------|------|----------|
| 重审定稿 | W1–W2 | 双候选（①②）同行级重审；期刊匹配矩阵定稿；确定主投+备投 | 重审报告 + 投稿决策备忘录 |
...
2. ~~候选 ② 依赖本轮 Chern-Simons 链 CLAIM→THEOREM 任务的结果~~（**已解除**：T3 已于 2026-08-21 登记第三项 THEOREM，双候选主线确认可行；T3′/T6/T7 补强条目待独立编译复核，不影响主线）；
3. D.4-5 参考文献工作依赖 DOI 拆分清理（已知遗留问题，可先绕行）。

### D.7 与现有框架的结合点

- **proof_status.md §三**（THEOREM 登记）与 §五（证明率口径修正）是候选 ① 的核心素材与诚信背书；
- **axiom_registry.json v2.0 + sweep1–4 报告（136 条清偿）** 提供候选 ① 的全部定量证据链；
...
|--------|------|------|------------------|----------|
| D1 | 投稿候选同行级重审（候选 ①②③④ 逐一评估，输出排序与主备投建议） | proof_status.md、axiom_registry.json、sweep1–4 报告、qec_ftqc、SYK-cMERA 素材 | `framework/submission_candidate_review.md` | 每候选含证据强度/新颖性/风险三维评分 |
...
|--------|------|------|------------------|----------|
| D2 | 期刊匹配矩阵定稿 + 投稿决策备忘录（含预印本政策核对） | D1 | `framework/submission_decision_memo.md` | 主投/备投/时间线三方落定 |
...
- **独占资源警示**：`proof_status.md` 为串行独占文件（今日 CS 链任务刚完成其 §三更新；后续任何登记仍须排队执行），CI 相关脚本与批量模块（sweep5 清偿进行中）在波次一/二均**不得触碰**，任务包产出一律走新增文件；
...
4. `framework/axiom_reduction_sweep1–4_report.md`（累计 136 条清偿的方法学记录）；
...
8. `framework/ci_slimming_phase1_report.md`（2026-08-21，生成器可复现性 + 5 模块迁移试点 + sorry-audit 30.2% 实测）；
9. 期刊信息（JAR/LMCS/JFR/MCS/CMP/JMP/LMP/J.Phys.A/PRD/JHEP/CQG/SIGMA 等）为公开出版常识，具体投稿要求以各刊官网为准。

*——本文件由搜索规划师于 2026-08-21 编制；任务切分建议供群管理员派发参考，不包含任何人员指派。*
